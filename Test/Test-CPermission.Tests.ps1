...
Describe 'Test-CPermission' {
    ...
    It 'should handle non existent path' {
        Test-CPermission -Path 'C:\I\Do\Not\Exist' -Identity $script:identity -Permission 'FullControl' -NoWarn -ErrorAction SilentlyContinue |
            Should -BeNullOrEmpty
        $Global:Error | Should -HaveCount 2
    }

    It 'should check ungranted permission on file system' {
        Test-CPermission @testDirPermArgs -Permission 'Write' -NoWarn | Should -BeFalse
    }

    It 'should check granted permission on file system' {
        Test-CPermission @testDirPermArgs -Permission 'Read' -NoWarn | Should -BeTrue
    }

    It 'should check exact partial permission on file system' {
        Test-CPermission @testDirPermArgs -Permission 'Read' -Exact -NoWarn | Should -BeFalse
    }

    It 'should check exact permission on file system' {
        Test-CPermission @testDirPermArgs -Permission 'ReadAndExecute' -Exact -NoWarn | Should -BeTrue
    }

    It 'should exclude inherited permission' {
        Test-CPermission @testFilePermArgs -Permission 'ReadAndExecute' -NoWarn | Should -BeFalse
    }

    It 'should include inherited permission' {
        Test-CPermission @testFilePermArgs -Permission 'ReadAndExecute' -Inherited -NoWarn | Should -BeTrue
    }

    It 'should exclude inherited partial permission' {
        Test-CPermission @testFilePermArgs -Permission 'ReadAndExecute' -Exact -NoWarn | Should -BeFalse
    }

    It 'should include inherited exact permission' {
        Test-CPermission @testFilePermArgs -Permission 'ReadAndExecute' -Inherited -Exact -NoWarn | Should -BeTrue
    }
    ...
    It 'should check ungranted permission on registry' {
        Test-CPermission -Path $script:keyPath -Identity $script:identity -Permission 'Delete' -NoWarn | Should -BeFalse
    }

    It 'should check granted permission on registry' {
        Test-CPermission -Path $script:keyPath -Identity $script:identity -Permission 'ReadKey' -NoWarn | Should -BeTrue
    }

    It 'should check exact partial permission on registry' {
        Test-CPermission -Path $script:keyPath -Identity $script:identity -Permission 'ReadKey' -Exact -NoWarn | Should -BeFalse
    }

    It 'should check exact permission on registry' {
        Test-CPermission -Path $script:keyPath -Identity $script:identity -Permission 'ReadKey','WriteKey' -Exact -NoWarn |
            Should -BeTrue
    }

    It 'should check ungranted inheritance flags' {
        Test-CPermission @testDirPermArgs -Permission 'ReadAndExecute' -ApplyTo ContainerAndSubContainersAndLeaves -NoWarn |
            Should -BeFalse
    }

    It 'should check granted inheritance flags' {
        Test-CPermission @testDirPermArgs -Permission 'ReadAndExecute' -ApplyTo ContainerAndLeaves -NoWarn |
            Should -BeTrue
        Test-CPermission @testDirPermArgs -Permission 'ReadAndExecute' -ApplyTo ChildLeaves -NoWarn | Should -BeTrue
    }


    It 'should check exact ungranted inheritance flags' {
        Test-CPermission @testDirPermArgs -Permission 'ReadAndExecute' -ApplyTo ContainerAndLeaves -Exact -NoWarn |
            Should -BeFalse
    }

    It 'should check exact granted inheritance flags' {
        Test-CPermission @testDirPermArgs -Permission 'ReadAndExecute' -ApplyTo ChildLeaves -Exact -NoWarn | Should -BeTrue
    }
    ...
    It 'should check permission on private key' {
        ...
        try
        {
            ...
            Grant-CPermission -Path $certPath -Identity $script:identity -Permission $allPerm -NoWarn
            Test-CPermission -Path $certPath -Identity $script:identity -Permission $readPerm -NoWarn | Should -BeTrue
            Test-CPermission -Path $certPath -Identity $script:identity -Permission $readPerm -Exact -NoWarn |
                Should -BeFalse
            Test-CPermission -Path $certPath -Identity $script:identity -Permission $allPerm, $readPerm -Exact -NoWarn |
                Should -BeTrue
        }
        finally
        {
    ...
    It 'should check permission on public key' -Skip:$script:usesFileSystemPermsOnPrivateKeys {
        ...
        Get-CPermission -path $certPath -Identity $script:identity -NoWarn | Out-String | Write-Host
        Test-CPermission -Path $certPath -Identity $script:identity -Permission 'FullControl' -NoWarn | Should -BeTrue
        Test-CPermission -Path $certPath -Identity $script:identity -Permission 'FullControl' -Exact -NoWarn |
            Should -BeTrue
    }
}
