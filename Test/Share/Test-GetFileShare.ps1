...
{
    $share = Invoke-CPrivateCommand -Name 'Get-CCimInstance' -Parameter @{Class = 'Win32_Share'; Filter = "Name='$shareName'"}
    if( $share -ne $null )
    {
        [void] $share.Delete()
    }
...
{
    $nonFileShares = Invoke-CPrivateCommand -Name 'Get-CCimInstance' -Parameter @{Class = 'Win32_Share'} |
