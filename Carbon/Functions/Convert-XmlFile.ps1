...
function Convert-CXmlFile
{
    ...
    else
    {
        ...
        $XdtPath = Resolve-CFullPath -Path $XdtPath -NoWarn
        $xdtPathForShouldProcess = $XdtPath
        $xdtPathForInfoMsg = 'with ''{0}'' ' -f $XdtPath
    }

    $Path = Resolve-CFullPath -Path $Path -NoWarn
    $Destination = Resolve-CFullPath -Path $Destination -NoWarn
    $TransformAssemblyPath = $TransformAssemblyPath | ForEach-Object { Resolve-CFullPath -Path $_ -NoWarn }
