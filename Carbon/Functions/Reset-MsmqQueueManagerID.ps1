...
function Reset-CMsmqQueueManagerID
{
    ...
    $QMIdName = "QMId"
   	$QMId = Get-CRegistryKeyValue -Path $QMIdPath -Name $QMIdName -NoWarn
   	Write-Verbose "Existing QMId: $QMId"
   	Remove-CRegistryKeyValue -Path $QMIdPath -Name $QMIdName -NoWarn

    $MSMQSysPrepPath = "HKLM:\SOFTWARE\Microsoft\MSMQ\Parameters"
    $MSMQSysPrepName = "SysPrep"
   	Remove-CRegistryKeyValue -Path $MSMQSysPrepPath -Name $MSMQSysPrepName -NoWarn
	Set-CRegistryKeyValue -Path $MSMQSysPrepPath -Name $MSMQSysPrepName -DWord 1 -NoWarn
